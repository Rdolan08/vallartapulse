...
if [[ -z "${DATABASE_URL:-}" && -z "${RAILWAY_DATABASE_URL:-}" && -f "$ROOT_DIR/.env" ]]; then
  set -a
...
if [[ -z "$DB_URL" ]]; then
  echo "ERROR: set DATABASE_URL or RAILWAY_DATABASE_URL (or add RAILWAY_DATABASE_URL to .env)" >&2
  exit 1
fi

CSV_PATH="${AIRBNB_PRICING_EXPORT_FILE:-$ROOT_DIR/airbnb_calendar_prices.csv}"
...
echo "[airbnb-pricing-sync] root=$ROOT_DIR"
echo "[airbnb-pricing-sync] export start"
pnpm --filter @workspace/scripts run scrape:airbnb-pricing-export
...
psql "$DB_URL" \
  -v ON_ERROR_STOP=1 \
  -v csv_path="$CSV_PATH" \
  -f "$SCRIPT_DIR/sql/load-airbnb-calendar-prices-safe.sql"
